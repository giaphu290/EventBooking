...
{
    public class GetAllowedEventGroupByGroupIdHandler : IRequestHandler<GetAllowedEventGroupByGroupIdQuery, IEnumerable<AllowedEventGroupResponse>>
    {
        private readonly IUnitOfWork _unitOfWork;
        private readonly IMapper _mapper;

        public GetAllowedEventGroupByGroupIdHandler(IUnitOfWork unitOfWork, IMapper mapper)
        {
            _unitOfWork = unitOfWork;
            _mapper = mapper;
        }

        public async Task<IEnumerable<AllowedEventGroupResponse>> Handle(GetAllowedEventGroupByGroupIdQuery request, CancellationToken cancellationToken)
        {
            try
            {
                var groupInvitations = await _unitOfWork.AllowedEventGroupRepository.GetAllowedEventGroupsByGroupIdAsync(request.GroupId)
