...
{
    public class GetEventInvitationByUserIdHandler : IRequestHandler<GetEventInvitationByUserIdQuery, IEnumerable<EventInvitationResponse>>
    {
        private readonly IUnitOfWork _unitOfWork;
        private readonly IMapper _mapper;

        public GetEventInvitationByUserIdHandler(IUnitOfWork unitOfWork, IMapper mapper)
        {
            _unitOfWork = unitOfWork;
            _mapper = mapper;
        }

        public async Task<IEnumerable<EventInvitationResponse>> Handle(GetEventInvitationByUserIdQuery request, CancellationToken cancellationToken)
        {

            try
            {
                var eventPosts = await _unitOfWork.EventInvitationRepository.GetEventInvitationsByUserIdAsync(request.UserId);
